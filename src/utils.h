...
public:
	...
	~ring_buffer() {
		for(std::size_t i = 0; i < size(); i++) {
			buffer[(front + i) % N].t.~T();
		}
	}
	ring_buffer(const T& other) = delete;
	ring_buffer(T&& other) = delete;
	ring_buffer& operator=(const T& other) = delete;
	ring_buffer& operator=(T&& other) = delete;
	void push(const T& t) {
		new (&buffer[(front + _size) % N]) T(t);
		if(_size == N) {
			front = (front + 1) % N;
		} else {
			_size++;
